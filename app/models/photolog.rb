class Photolog < ActiveRecord::Base
  ...
  def up_photo(params)
    ...
    Photolog.connection.execute("INSERT INTO #{self.table_name} VALUES (DEFAULT, '#{user_id}', #{user_route_id}, #{user_photo_id}, #{photo_lat}, #{photo_lon}, '#{[shoot_time.split(' ')[0], shoot_time.split(' ')[1]].join(' ')}', '', '#{memo}', '#{geo_tag}')")
